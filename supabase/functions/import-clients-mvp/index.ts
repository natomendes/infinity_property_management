...
serve(async (req) => {
  try {
    ...
    async function fetchStaysNetClients(token: string, skip: number, limit: number) {
    ...
    while (hasMore) {
      const clientData = await fetchStaysNetClients(accessToken, currentSkip, fetchLimit);
    ...
    function transformClientData(staysNetClient: any): any {
      ...
      const phone = staysNetClient.phones && staysNetClient.phones.length > 0 ? staysNetClient.phones[0].number : null;
      const documentId = staysNetClient.documents && staysNetClient.documents.length > 0 ? staysNetClient.documents[0].number : null;
    ...
    if (allClients.length > 0) {
      ...
      console.log(`Successfully upserted ${upsertedData ? upsertedData.length : 0} clients to Supabase 'owners' table.`);
      const responseData = {
